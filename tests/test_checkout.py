...
def test_checkout_with_missing_first_name(driver):
    config = ConfigReader.read_config()
    ...
    checkout_page.fill_checkout_information("", "Munawar", "12345")

    assert "First Name is required" in checkout_page.get_error_message()


def test_checkout_with_missing_last_name(driver):
    config = ConfigReader.read_config()
    ...
    checkout_page.fill_checkout_information("Usman", "", "12345")

    assert "Last Name is required" in checkout_page.get_error_message()


def test_checkout_with_missing_postal_code(driver):
    config = ConfigReader.read_config()
    ...
    checkout_page.fill_checkout_information("Usman", "Munawar", "")
